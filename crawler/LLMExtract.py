...
def shiet():
    ...
    # Đọc dữ liệu Delta từ MinIO và chuyển thành DeltaTable

    # Đảm bảo rằng cột 'category' có trong bảng target (nếu chưa có, thêm vào)
  
    # Đọc dữ liệu mới từ Delta
    df_new = spark.read.format("delta").load("s3a://datn/master/recruit")  # Lấy 3 bản ghi đầu tiên

    # Dùng `extract_from_pdf` để thêm cột `category` cho các bản ghi có giá trị trong cột `industries`
    df_new = df_new.withColumn(
        "category",
        F.when(F.col("industries").isNotNull(), extract_from_pdf(F.col("industries"))).otherwise(None)
    )
