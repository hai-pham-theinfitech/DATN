...
def identify_person_relocation(
    ...
    identify_method: IdentifyPersonRelocation = IdentifyPersonRelocation.MASTER_NAME_DOMAIN,
):
    ...
    elif identify_method == IdentifyPersonRelocation.MASTER_COMPANY_DOMAIN:
        df = match_company_domain(company_df, master_df, column = "company_id")
    elif identify_method == IdentifyPersonRelocation.MASTER_PROVINCE:
        df = match_company_province(company_df, master_df, column = "company_id")
    elif identify_method == IdentifyPersonRelocation.MASTER_WARD:
        from match_company import match_company_ward
        df = match_company_ward(company_df, master_df, column = "company_id")
    elif identify_method == IdentifyPersonRelocation.MASTER_WARD_PROVINCE:
    ...
    elif identify_method == IdentifyPersonRelocation.MASTER_NAME_DOMAIN:
        from match_company import match_company_name_domain
        df = match_company_name_domain(company_df, master_df, column = "company_id")
    elif identify_method == IdentifyPersonRelocation.MASTER_STREET_PROVINCE:
    ...
    elif identify_method == IdentifyPersonRelocation.MASTER_STREET_WARD_PROVINCE:
...
for method in IdentifyPersonRelocation:
    print(f"Identify method: {method}")
    identify_person_relocation(identify_method=method)
# identify() 
