...
def shiet():
    ...
    df = df.withColumn(
        "edu",
        F.when(
            F.col("requirements").rlike("(?i)THCS|Cấp 2|Trung học Cơ sở|TH Cơ sở|secondary"), "THCS"
        ).when(
            F.col("requirements").rlike("(?i)THPT|Trung học Phổ Thông|cấp 3|TH Phổ thông|phổ thông| high school"), "THPT"
        ).when(
            F.col("requirements").rlike("(?i)Trung cấp|tc"), "Trung cấp"
        ).when(
            F.col("requirements").rlike("(?i)Cao đẳng|cđ"), "Cao đẳng"
        ).when(
            F.col("requirements").rlike("(?i)Cử nhân|kĩ sư|college|Bachelor|degree|majors|University"), "Đại học"
        ).otherwise("Đại học")  
    )
    df.write \
        .format("delta") \
